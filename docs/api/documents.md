...
②で S3 に上げた後、メタデータを登録する。`folder_ids` は所属フォルダ（**最大1件**。1書類=1フォルダの不変条件。省略/空配列なら未分類）。`tags` は名前で受け取り、サーバー側で upsert する。
...
`id` は任意。バックグラウンドアップロードの楽観的表示のため、クライアントで採番した uuid を渡せる（省略時はサーバーで採番）。同じ `id`・同じ `s3_key` の書類が既にある場合は再試行とみなして既存の書類を返す（冪等）。`id` だけが衝突した場合は `409`。
...
送られたフィールドのみ更新。`folder_ids` / `tags` を渡すと**差し替え**（全置換）。`folder_ids` は最大1件（フォルダ移動もこの経路。空配列で未分類へ）。`updated_by` はセッションの user を設定する。
