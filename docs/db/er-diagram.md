...
第1フェーズのテーブル関係。タグは書類と多対多、フォルダは書類と所属テーブルで結ぶ（構造は多対多だがアプリ層で 1書類=最大1件に制約）。フォルダは自己参照で階層を持つ。
...
- **documents ⇄ folders**: `document_folders` が所属の実体。構造は多対多だがアプリ層で 1書類=最大1件に制約（未分類 = 0件）。
