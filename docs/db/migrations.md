...
- 適用済みは drizzle の journal（`__drizzle_migrations`）で管理。**local と本番で同じ仕組み**なので、ローカル(docker)で通ったマイグレーションがそのまま本番に流れる。
- ローカルを作り直したいときは `docker compose down -v` で破棄 →`up -d`→`pnpm db:migrate` で最新まで再適用。
- `pnpm db:generate` / `db:migrate` / `db:studio` スクリプトと `drizzle.config.ts` は用意済み（`package.json`）。

## 環境モデル（決定：開発=ローカル docker / 本番=Neon）

開発は各自のローカル docker(`compose.yaml`)で完結する。上位 DB は **Neon 本番の1つのみ**（staging ブランチは持たない）。

```
PR 作成/更新 ─► [check]    drizzle-kit のドリフト検査（schema.ts と生成済み migration の不整合を検知。DB 非接続）
main merge 後 ─► [migrate]  手動実行で Neon 本番へ drizzle-kit migrate（Environment: production で保護）
```

## GitHub Actions

- ワークフロー配置: `.github/workflows/`。ドリフト検査は `db-check.yml`、適用は `db-migrate.yml`。
- **PR（ドリフト検査 / `db-check.yml`）**: `pnpm db:generate` 後に `drizzle/` に差分が出ないことを確認。差分があれば「migration 未生成」または「手書き SQL とのズレ」として fail させる。DB へは接続・適用しない。自動実行（PR → develop / main）。
- **本番適用（`db-migrate.yml`）**: **手動トリガー（`workflow_dispatch`）**。`Environment: production` の `DATABASE_URL_UNPOOLED` で `drizzle-kit migrate`。main をマージ後に Actions タブから実行する。production 環境は保護ルール（必要なら承認）を付ける。
- **同時実行制御**: `concurrency` で本番への並行適用を防ぐ（適用中はキャンセルしない）。
...
- 破壊的変更（列削除・型変更）は、まずローカル docker で検証し、必要なら「追加 → データ移行 → 旧削除」の多段に分ける。
