...
設計の中核: **ファイル実体は S3 にフラット保存し、フォルダ・分類はすべてメタデータ (DB) で表現する。** フォルダは物理ディレクトリではなく「階層を持てるラベル」である。

**不変条件: 1書類は高々1つのフォルダに所属する（未分類 = 0個）。** 分類軸として複数持てるのはタグの役割とする。DB は将来の拡張余地として `document_folders`（多対多）テーブルのまま持つが、アプリ層で所属を最大1件に制約する（登録・移動 UI は単一選択、`folder_ids` は最大1要素）。
...
残りはシステムが裏で持つ項目。フォルダとタグはこのテーブルには持たず、別表 (document_folders / document_tags) で表現する（フォルダは 1書類=最大1件、タグは多対多）。
...
### 1.4 document_folders — 書類⇄フォルダ (所属テーブル)
...
- 複合主キー `PRIMARY KEY (document_id, folder_id)`。テーブル構造は多対多だが、**アプリ層で 1書類あたり最大1行に制約する**（上記の不変条件）。
- 1行も無い書類が「未分類」。
