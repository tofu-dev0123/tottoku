      ...
      <div className="w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="px-5 pt-8 pb-6 text-center">
          <Image
            ...
            width={745}
            height={280}
            className="mx-auto h-auto w-28 select-none"
          />
        ...
        <div className="px-5 pb-4">
          <div className="flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-xs font-medium text-gray-800">
            <GoogleGlyph />
            Google でログイン
          </div>
  ...
  return (
    <div className="flex h-full flex-col bg-gray-50">
      <header className="flex items-center justify-between px-4 pt-8 pb-3">
        <div>
          <p className="text-[11px] text-gray-500">おかえりなさい</p>
          <p className="mt-0.5 text-base font-medium text-gray-900">わが家の書類</p>
        </div>
      ...
      <div className="px-4">
        <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-gray-400">
          <Search className="size-4" />
          <span className="text-xs">書類を検索</span>
        </div>
      </div>

      <div className="px-4 pt-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-xs font-medium text-gray-900">期限が近い書類</span>
          ...
          {items.map((d) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-[10px]">期限まで あと {d.left}日</p>
              </div>
      ...
      <div className="mt-auto flex items-center justify-around border-t border-gray-100 bg-white px-2 py-2.5">
...
function MockTab({
  ...
  return (
    <div
      className={`flex flex-col items-center gap-0.5 ${active ? "text-blue-700" : "text-gray-400"}`}
    >
      {icon}
...
function FilerMock() {
  ...
  const docs = [
    { title: "自動車保険 証券", tag: "保険", multi: true },
    { title: "賃貸借契約書", tag: "住まい・車", multi: false },
    { title: "こども医療費受給者証", tag: "子ども", multi: true },
    { title: "健康診断結果 2025", tag: "健康・医療", multi: false },
    { title: "確定申告 控え", tag: "契約・行政", multi: false },
    { title: "火災保険 証券", tag: "保険", multi: true },
  ];
  return (
    <div className="flex h-[420px]">
      {/* サイドバー */}
      <aside className="hidden w-56 shrink-0 flex-col border-r border-gray-100 bg-[#f1f2f4] px-3 py-4 sm:flex">
        <Image
          src="/tottoku-gray.png"
          alt="トットク"
          width={751}
          height={283}
          className="mb-5 h-6 w-auto self-start pl-1 select-none"
        />
        <div className="flex items-center gap-2 rounded-lg bg-blue-700 px-3 py-2 text-sm font-medium text-white">
          <Home className="size-4" />
          わが家の書類
        </div>
        <p className="mt-5 mb-1 px-3 text-[11px] font-medium tracking-wide text-gray-400">
          フォルダ
        </p>
        <ul className="space-y-0.5">
          {folders.map((f) => (
            <li
              key={f}
              className="flex items-center gap-2 rounded-md px-3 py-1.5 text-sm text-gray-600"
            >
      ...
      <div className="flex-1 overflow-hidden bg-white px-6 py-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">わが家の書類</h3>
            <p className="mt-0.5 text-xs text-gray-400">全 {docs.length} 件</p>
          </div>
          <div className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-400">
            <Search className="size-4" />
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-3">
          {docs.map((d) => (
            <div
              key={d.title}
              className="rounded-xl border border-gray-200 bg-white p-3.5 transition-colors hover:border-blue-200 hover:bg-blue-50/30"
            >
              ...
              <p className="mt-3 truncate text-sm font-medium text-gray-900">{d.title}</p>
              <p className="mt-1 truncate text-xs text-gray-400">{d.tag}</p>
            </div>
          ))}
        </div>
      </div>
